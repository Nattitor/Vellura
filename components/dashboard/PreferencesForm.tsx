    ...
    <div className="w-full ethereal-panel p-6 md:p-8 rounded-xl flex flex-col space-y-6 relative overflow-hidden">
      ...
      <div className="space-y-5 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-white/5">
          <div className="flex items-center space-x-3">
            <div className="w-9 h-9 rounded-full bg-zinc-900/80 border border-white/5 flex items-center justify-center">
              <Languages className="w-4 h-4 text-zinc-400" />
            </div>
            <div>
              <p className="text-white font-medium text-sm">{t.settings.uiLang}</p>
              <p className="text-xs text-zinc-500">{t.settings.uiLangDesc}</p>
            </div>
          </div>
          <Select value={uiLanguage} onValueChange={(val) => setUiLanguage(val as LanguageType)}>
            <SelectTrigger className="w-[170px] h-9 text-xs bg-zinc-900/50 border-white/10 text-white focus:ring-amethyst-glow transition-all">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-white/5">
          <div className="flex items-center space-x-3">
            <div className="w-9 h-9 rounded-full bg-amethyst-glow/10 flex items-center justify-center">
              <Languages className="w-4 h-4 text-amethyst-glow" />
            </div>
            <div>
              <p className="text-white font-medium text-sm">{t.settings.outputLang}</p>
              <p className="text-xs text-zinc-500">{t.settings.outputLangDesc}</p>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-white font-medium text-sm">{t.settings.theme}</p>
              <p className="text-xs text-zinc-500">{t.settings.themeDesc}</p>
            </div>
          </div>
          <Button disabled variant="outline" className="bg-zinc-900/50 border-white/10 text-zinc-400 w-[170px] h-9 text-xs justify-start opacity-70">
            Dark Mode (Enforced)
          </Button>
        </div>
      </div>
      ...
      <div className="flex justify-end pt-3 relative z-10">
        <Button
          onClick={handleSave}
          disabled={isPending}
          className="bg-amethyst-glow hover:bg-amethyst-glow/90 text-white active:scale-[0.98] transition-all min-w-[120px] shadow-[0_0_15px_rgba(139,92,246,0.3)] hover:shadow-[0_0_20px_rgba(139,92,246,0.5)] cursor-pointer"
        >
